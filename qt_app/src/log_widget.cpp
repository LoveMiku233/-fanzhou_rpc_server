...
void LogWidget::appendLog(const QString &message, const QString &level)
{
    ...
    if (totalCount_ > kMaxLogEntries) {
        // 批量删除,减少重绘次数
        const int linesToRemove = kMaxLogEntries / 4;  // 删除25%
        QTextCursor cursor(logTextEdit_->document());
        ...
        cursor.endEditBlock();
        totalCount_ = qMax(0, totalCount_ - linesToRemove);
        warningCount_ = qMax(0, warningCount_ - linesToRemove / 3);  // 估算
        errorCount_ = qMax(0, errorCount_ - linesToRemove / 5);    // 估算
    }
