...
void SettingsWidget::onGetMqttConfig()
{
    ...
    // 新版RPC：通过 config.get 读取 mqttChannels 配置
    QJsonValue cfgResult = rpcClient_->call(QStringLiteral("config.get"));
    if (!cfgResult.isObject()) {
        QMessageBox::warning(this, QStringLiteral("错误"), QStringLiteral("读取配置失败: 返回格式错误"));
        return;
    }
    const QJsonObject cfgObj = cfgResult.toObject();
    if (!cfgObj.value(QStringLiteral("ok")).toBool()) {
        const QString err = cfgObj.value(QStringLiteral("error")).toString();
        QMessageBox::warning(this, QStringLiteral("错误"), QStringLiteral("读取配置失败: %1").arg(err));
        return;
    }

    const QJsonArray channels = cfgObj.value(QStringLiteral("mqttChannels")).toArray();
    QJsonObject channelCfg;
    for (const QJsonValue &v : channels) {
        if (!v.isObject()) continue;
        QJsonObject obj = v.toObject();
        if (obj.value(QStringLiteral("channelId")).toInt() == 1) {
            channelCfg = obj;
            break;
        }
        if (channelCfg.isEmpty()) {
            channelCfg = obj;
        }
    }

    if (channelCfg.isEmpty()) {
        mqttEnabledCheckBox_->setChecked(false);
        mqttBrokerEdit_->clear();
        mqttPortSpinBox_->setValue(1883);
        mqttClientIdEdit_->clear();
        mqttUsernameEdit_->clear();
        mqttPasswordEdit_->clear();
        mqttTopicEdit_->clear();
        mqttStatusLabel_->setText(QStringLiteral("MQTT状态: 未配置通道（channelId=1）"));
        mqttStatusLabel_->setStyleSheet(QStringLiteral(
            "font-size: 12px; padding: 8px; background-color: #fff3cd; color: #856404; border-radius: 6px;"));
        emit logMessage(QStringLiteral("MQTT通道未配置"), QStringLiteral("WARN"));
        return;
    }

    const int channelId = channelCfg.value(QStringLiteral("channelId")).toInt(1);
    mqttEnabledCheckBox_->setChecked(channelCfg.value(QStringLiteral("enabled")).toBool(true));
    mqttBrokerEdit_->setText(channelCfg.value(QStringLiteral("broker")).toString());
    mqttPortSpinBox_->setValue(channelCfg.value(QStringLiteral("port")).toInt(1883));
    mqttClientIdEdit_->setText(channelCfg.value(QStringLiteral("clientId")).toString());
    mqttUsernameEdit_->setText(channelCfg.value(QStringLiteral("username")).toString());
    mqttPasswordEdit_->setText(channelCfg.value(QStringLiteral("password")).toString());
    mqttTopicEdit_->setText(channelCfg.value(QStringLiteral("topicPrefix")).toString());

    // 读取运行态连接状态
    QJsonValue statusResult = rpcClient_->call(QStringLiteral("mqtt.channels.list"));
    bool connected = false;
    if (statusResult.isObject()) {
        const QJsonArray statusArr = statusResult.toObject().value(QStringLiteral("channels")).toArray();
        for (const QJsonValue &v : statusArr) {
            if (!v.isObject()) continue;
            const QJsonObject s = v.toObject();
            if (s.value(QStringLiteral("channelId")).toInt() == channelId) {
                connected = s.value(QStringLiteral("connected")).toBool(false);
                break;
            }
        }
    }

    const bool enabled = mqttEnabledCheckBox_->isChecked();
    if (enabled && connected) {
        mqttStatusLabel_->setText(QStringLiteral("MQTT状态: 已连接 (channel %1)").arg(channelId));
        mqttStatusLabel_->setStyleSheet(QStringLiteral(
            "font-size: 12px; padding: 8px; background-color: #d4edda; color: #155724; border-radius: 6px;"));
    } else if (enabled) {
        mqttStatusLabel_->setText(QStringLiteral("MQTT状态: 已启用但未连接 (channel %1)").arg(channelId));
        mqttStatusLabel_->setStyleSheet(QStringLiteral(
            "font-size: 12px; padding: 8px; background-color: #fff3cd; color: #856404; border-radius: 6px;"));
    } else {
        mqttStatusLabel_->setText(QStringLiteral("MQTT状态: 未启用 (channel %1)").arg(channelId));
        mqttStatusLabel_->setStyleSheet(QStringLiteral(
            "font-size: 12px; padding: 8px; background-color: #e9ecef; color: #495057; border-radius: 6px;"));
    }

    emit logMessage(QStringLiteral("MQTT配置读取成功（新RPC接口）"));
}
...
void SettingsWidget::onSetMqttConfig()
{
    ...
    const bool enabled = mqttEnabledCheckBox_->isChecked();
    const QString broker = mqttBrokerEdit_->text().trimmed();
    if (enabled && broker.isEmpty()) {
        QMessageBox::warning(this, QStringLiteral("警告"), QStringLiteral("启用MQTT时 Broker 地址不能为空"));
        return;
    }

    // 兼容新RPC：remove -> add -> connect/disconnect
    QJsonObject removeParams;
    removeParams[QStringLiteral("channelId")] = channelId;
    rpcClient_->call(QStringLiteral("mqtt.channels.remove"), removeParams);  // 忽略失败，可能本就不存在

    QJsonObject addParams;
    addParams[QStringLiteral("channelId")] = channelId;
    addParams[QStringLiteral("name")] = QStringLiteral("qt-app-channel-1");
    addParams[QStringLiteral("type")] = 0;
    addParams[QStringLiteral("enabled")] = enabled;
    addParams[QStringLiteral("broker")] = broker;
    addParams[QStringLiteral("port")] = mqttPortSpinBox_->value();
    addParams[QStringLiteral("clientId")] = mqttClientIdEdit_->text().trimmed();
    addParams[QStringLiteral("username")] = mqttUsernameEdit_->text().trimmed();
    addParams[QStringLiteral("password")] = mqttPasswordEdit_->text();
    addParams[QStringLiteral("topicPrefix")] = mqttTopicEdit_->text().trimmed();
    addParams[QStringLiteral("qos")] = 0;
    addParams[QStringLiteral("keepAliveSec")] = 60;
    addParams[QStringLiteral("autoReconnect")] = true;
    addParams[QStringLiteral("reconnectIntervalSec")] = 5;

    QJsonValue addResult = rpcClient_->call(QStringLiteral("mqtt.channels.add"), addParams);
    if (!addResult.isObject() || !addResult.toObject().value(QStringLiteral("ok")).toBool()) {
        const QString error = addResult.toObject().value(QStringLiteral("error")).toString();
    ...
    connParams[QStringLiteral("channelId")] = channelId;
    if (enabled) {
        rpcClient_->call(QStringLiteral("mqtt.channels.connect"), connParams);
    } else {
        rpcClient_->call(QStringLiteral("mqtt.channels.disconnect"), connParams);
    }

    // 持久化
    QJsonValue saveResult = rpcClient_->call(QStringLiteral("config.save"));
    const bool saved = saveResult.isObject() && saveResult.toObject().value(QStringLiteral("ok")).toBool();

    if (saved) {
        QMessageBox::information(this, QStringLiteral("成功"), QStringLiteral("MQTT配置保存成功（新RPC接口）！"));
    } else {
        QMessageBox::warning(this, QStringLiteral("警告"),
                             QStringLiteral("MQTT运行时配置已更新，但写入文件失败，请手动执行 config.save"));
    }
    emit logMessage(QStringLiteral("MQTT配置保存成功（channelId=1）"));
    onGetMqttConfig();
}
...
void SettingsWidget::onTestMqtt()
{
    ...
    const int channelId = 1;
    QJsonObject params;
    params[QStringLiteral("channelId")] = channelId;
    QJsonValue connResult = rpcClient_->call(QStringLiteral("mqtt.channels.connect"), params);

    if (connResult.isObject() && connResult.toObject().value(QStringLiteral("ok")).toBool()) {
        QJsonValue listResult = rpcClient_->call(QStringLiteral("mqtt.channels.list"));
        bool connected = false;
        if (listResult.isObject()) {
            const QJsonArray channels = listResult.toObject().value(QStringLiteral("channels")).toArray();
            for (const QJsonValue &v : channels) {
                if (!v.isObject()) continue;
                const QJsonObject obj = v.toObject();
                if (obj.value(QStringLiteral("channelId")).toInt() == channelId) {
                    connected = obj.value(QStringLiteral("connected")).toBool(false);
                    const QString broker = obj.value(QStringLiteral("broker")).toString();
                    const int port = obj.value(QStringLiteral("port")).toInt();
                    QMessageBox::information(
                        this, QStringLiteral("MQTT测试"),
        ...
        emit logMessage(QStringLiteral("MQTT测试完成（connected=%1）").arg(connected));
    } else {
        const QString err = connResult.toObject().value(QStringLiteral("error")).toString();
        QMessageBox::warning(this, QStringLiteral("MQTT测试失败"), QStringLiteral("连接命令失败: %1").arg(err));
        emit logMessage(QStringLiteral("MQTT测试失败: %1").arg(err), QStringLiteral("ERROR"));
    }
}
...
void SettingsWidget::onShutdownSystem()
{
    QMessageBox::information(
        ...
        QStringLiteral("当前RPC服务器未提供 sys.shutdown 接口。\n请使用 sys.reboot 或在系统层手动执行关机。"));
    emit logMessage(QStringLiteral("sys.shutdown 未实现，已提示用户"), QStringLiteral("WARN"));
}
