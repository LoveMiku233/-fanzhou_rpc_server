...
{
    Q_UNUSED(nodeId);
    // 2026-04: 最新RPC服务器未提供 sensor.getParams，参数展示走 sensor.list/sensor.read 返回字段。
    static bool warned = false;
    if (!warned) {
        warned = true;
        qDebug() << "[SENSOR_WIDGET] sensor.getParams disabled: method not available on current RPC server";
        emit logMessage(QStringLiteral("sensor.getParams 已屏蔽，使用 sensor.list/sensor.read"), QStringLiteral("WARN"));
    }
}
