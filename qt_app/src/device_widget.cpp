...
DeviceWidget::DeviceWidget(RpcClient *rpcClient, QWidget *parent)
    ...
    , isQueryingAll_(false)
    , pendingStatusRequests_(0)
    , lastStatusRefreshTime_(0)
{
    setupUi();
...
void DeviceWidget::setToolbarBusy(bool busy)
{
    if (refreshButton_) refreshButton_->setEnabled(!busy);
    if (queryAllButton_) queryAllButton_->setEnabled(!busy);
    if (addDeviceButton_) addDeviceButton_->setEnabled(!busy);
}

void DeviceWidget::finishStatusRefreshOne()
{
...
void DeviceWidget::refreshDeviceList()
{
    ...
    // 使用异步调用避免阻塞UI线程
    int reqId = rpcClient_->callAsync(QStringLiteral("device.list"), QJsonObject(), this,
...
void DeviceWidget::tryRelayNodesAsFallback()
{
    ...
    if (reqId < 0) {
        statusLabel_->setText(QStringLiteral("[X] 获取失败"));
        emit logMessage(QStringLiteral("获取设备列表失败：无法发送请求"), QStringLiteral("ERROR"));
        isRefreshing_ = false;
        setToolbarBusy(false);
    }
}
...
void DeviceWidget::onQueryAllClicked()
{
    ...
    if (isQueryingAll_) {
        qDebug() << "[DEVICE_WIDGET] queryAll进行中，跳过";
        return;
    }
    isQueryingAll_ = true;
    setToolbarBusy(true);
    ...
    // 使用异步调用避免阻塞UI线程
    const int reqId = rpcClient_->callAsync(QStringLiteral("relay.queryAll"), QJsonObject(), this,
        [this](const QJsonValue &result, const QJsonObject &error) {
            isQueryingAll_ = false;
            setToolbarBusy(false);
            if (!error.isEmpty()) {
    ...
    if (reqId < 0) {
        ...
        setToolbarBusy(false);
        statusLabel_->setText(QStringLiteral("[X] 发送请求失败"));
        emit logMessage(QStringLiteral("查询所有设备失败：无法发送请求"), QStringLiteral("ERROR"));
    }
}
...
void DeviceWidget::onAddDeviceClicked()
{
    ...
    // 使用异步调用避免阻塞UI线程
    rpcClient_->callAsync(QStringLiteral("device.add"), params, this,
